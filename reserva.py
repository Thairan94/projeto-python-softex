...
from banco import carregar_reservas_do_banco, salvar_reserva_no_banco, deletar_reserva_do_banco # CORRIGIDO: deletar_reserva_do_banco
...
class AppReservaSalas(QWidget):
    ...
    def __init__(self, janela_anterior, nome_usuario, matricula_usuario):
        ...
        # CORREÇÃO: Chamando o método com a ortografia correta ('actualizar')
    ...
    def reservar_sala(self, sala, slot):
        ...
        if info_reserva is not None and info_reserva['matricula'] == self.matricula_usuario:
            ...
            resposta = QMessageBox.question(self, "Confirmação de Cancelamento",
                                            ...
                                            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No) # CORRIGIDO: StandardButton.No
            
            if resposta == QMessageBox.StandardButton.Yes:
                # Chama a função do banco para deletar o registro.
                if deletar_reserva_do_banco(sala, slot): # CORRIGIDO: deletar_reserva_do_banco
        ...
        # Checa se o usuário logado JÁ TEM esse slot reservado em QUALQUER OUTRA sala.
        for outra_sala in self.salas:
            # Se for uma sala diferente da que está sendo reservada E o slot estiver ocupado...
            if outra_sala != sala and self.salas[outra_sala][slot] is not None:
                # ... e se a reserva for do usuário logado (ele reservou outra sala neste horário)
                if self.salas[outra_sala][slot]['matricula'] == self.matricula_usuario:
        ...
        # 2.2 VERIFICAÇÃO DE SLOT OCUPADO POR OUTRO USUÁRIO
        if info_reserva is not None:
            # Slot ocupado por outro usuário (aqui info_reserva['matricula'] != self.matricula_usuario)
            ...
            return # Sai da função após avisar que o slot está ocupado

        # 2.3 VERIFICAÇÃO DE LIMITE DE 2 RESERVAS (Se o slot estiver livre e sem conflito)
        ...
        # 2.4 Faz a nova reserva (O slot está livre, sem conflitos e dentro do limite)
